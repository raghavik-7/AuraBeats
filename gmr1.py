...
class MusicRecommender:
    ...
    def recommend_songs(self, image_caption: str, user_input: str = "", context: str = "", 
                       num_songs: int = 6, preferred_languages: str = "", 
        ...
        try:
            ...
            comprehensive_data = self._get_comprehensive_recommendations(
                image_caption, user_input, context, num_songs, 
            ...
            final_recommendations = self._merge_recommendations(
                spotify_recommendations, gemini_recommendations, scene_analysis, num_songs
            )
    ...
    def _get_comprehensive_recommendations(self, image_caption: str, user_input: str, context: str, 
                                         num_songs: int, preferred_languages: str, 
                                         additional_preferences: str) -> Dict[str, Any]:
...
1. Scene analysis
2. Spotify search keywords for finding trending songs
3. {num_songs} specific, real song recommendations
...
TASK 1 - SPOTIFY KEYWORDS: Generate 2-3 short, evocative keywords or phrases (not generic music terms like "acoustic," "instrumental," or "background") that, when searched on Spotify, will find songs that perfectly match the mood, character, or scene of the image. Focus on keywords that capture the essence of the scene, such as emotions, character traits, or visual themes (e.g., "boss lady," "confidence," "she," "soft pop," "girl power," "nostalgia," "sundar pal," "sukoon," etc.), and adapt the keywords to the user's preferred language if specified. Never include English keywords if the language preference is set to a non-English language. Always use lowercase.

TASK 2 - SCENE ANALYSIS: Analyze the primary mood, visual elements, atmosphere, energy level, and setting type.

TASK 3 - SONG RECOMMENDATIONS: Recommend {num_songs} specific, real, POPULAR songs that are likely trending on Instagram and Spotify charts. Focus on songs that feel authentic and natural for the scene.

IMPORTANT: Keep song titles SHORT and CLEAN. Do not include explanations or additional text in the song title field.

Respond with this EXACT JSON format:
{{
    "spotify_keywords": ["keyword1", "keyword2", "keyword3"],
    "scene_analysis": {{
...
Ensure all three sections are properly filled out. The spotify_keywords should be optimized for finding trending songs that match the scene mood. If additional preferences are provided, ensure they significantly influence both keywords and recommendations while maintaining relevance to the image.
"""
    ...
    def _search_spotify_with_keywords(self, keywords: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Search Spotify for each keyword
            for keyword in keywords[:3]:  # Limit to 3 keywords to avoid rate limiting
                try:
                    results = self.sp.search(q=keyword, type="track", market="IN", limit=5)
                    items = results.get("tracks", {}).get("items", [])
                    
                    for track in items:
                        # Only add popular tracks (popularity >= 40)
            ...
            spotify_tracks.sort(key=lambda x: x["popularity"], reverse=True)
            
            logger.info(f" Found {len(spotify_tracks)} Spotify tracks")
            return spotify_tracks[:6]  # Return top 6 tracks for merging
    ...
    def _merge_recommendations(self, spotify_tracks: List[Dict[str, Any]], 
        ...
        for track in spotify_tracks:
            ...
            if song_key not in seen_songs and len(final_result["recommendations"]) < num_songs:
        ...
        for song in gemini_recommendations:
            ...
            if song_key not in seen_songs and song_title and len(final_result["recommendations"]) < num_songs:
                seen_songs.add(song_key)
...
def test_recommender():
    ...
    try:
        ...
        print(f"Scene Analysis: {results.get('spotify_keywords', {})}")
        print("\nRecommendations:")
